...
当前 Web UI 正在逐步迁移到保守型前端架构：

- `Jinja2` 负责服务端模板渲染
- `htmx` 负责局部刷新
- `Alpine.js` 负责轻量交互状态
- `TypeScript + Vite + SCSS` 负责前端资源组织与构建
...
- 如果未执行前端构建，页面仍会回退到现有 `/static/app.css` 样式与基础交互，不会阻塞后端运行。
- 当前 `ask` 页面已开始迁移到新架构，其余页面会逐步迁移。
- 只有当你显式设置 `KB_FRONTEND_ASSETS=true` 时，服务端才会注入 `frontend/dist` 下的构建产物，避免迁移期新旧样式冲突。
- 推荐验收顺序：`dashboard -> inbox -> search -> ask -> knowledge -> review -> settings`。
